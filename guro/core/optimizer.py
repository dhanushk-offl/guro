...
class GPUOptimizer:
    ...
    def _check_intel(self) -> bool:
        if platform.system() == 'Windows':
            # Check for Intel GPU using Windows Management Instrumentation
            try:
                output = subprocess.check_output('wmic path win32_VideoController get name', shell=True)
                return 'Intel' in output.decode()
            except subprocess.CalledProcessError:
                return False
        else:
            # Linux check
            vendors = ['/sys/class/drm/card0/device/vendor']
            for vendor_path in vendors:
                if os.path.exists(vendor_path):
                    try:
                        with open(vendor_path, 'r') as f:
                            vendor_id = f.read().strip()
                            return vendor_id == '0x8086'  # Intel vendor ID
                    except:
                        continue
            return False
